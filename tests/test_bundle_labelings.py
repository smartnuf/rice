...
def test_single_edge_support_every_assignment_is_canonical():
    ...
    assert (
        simple_bundle_labeling_orbit_count(g, (0, 1), max_r=3, max_reactive=5)
        == 7
    )
...
def test_two_edge_terminal_path_identifies_reversal_pairs():
    ...
    assert (
        simple_bundle_labeling_orbit_count(g, (0, 2), max_r=3, max_reactive=5)
        == 28
    )
...
def test_support_symmetry_not_preserving_terminal_set_is_excluded():
    ...
    assert simple_bundle_labeling_orbit_count(
        g, (0, 1), max_r=3, max_reactive=5
    ) == _brute_orbit_count(g, (0, 1), 3, 5)
    assert simple_bundle_labeling_orbit_count(g, (0, 1), max_r=3, max_reactive=5) < 7**3
...
def test_budget_accounting_multiplies_bundle_weight_by_edge_cycle_length():
    ...
    assert simple_bundle_labeling_orbit_count(g, (0, 2), max_r=1, max_reactive=2) == 10
...
def test_phase_2_raw_and_phase_3_canonical_standard_totals():
    result = simple_bundle_labeling_census(max_r=3, max_reactive=5)
...
def test_simple_bundle_labeling_census_zero_budget_is_empty():
    result = simple_bundle_labeling_census(max_r=0, max_reactive=0)
...
def test_simple_bundle_labeling_census_rejects_negative_budgets_and_zero_edges_when_nonempty():
    with pytest.raises(ValueError, match="non-negative"):
        simple_bundle_labeling_census(max_r=-1, max_reactive=0)
    with pytest.raises(ValueError, match="non-negative"):
        simple_bundle_labeling_census(max_r=0, max_reactive=-1)
    with pytest.raises(ValueError, match="at least 1"):
        simple_bundle_labeling_census(max_r=1, max_reactive=0, max_edges=0)
